...
predicted_O_tanh = scm_optim.predict_O_tanh(B, X, contact_subgraph)
...
predicted_O_rectified_tanh = scm_optim.predict_O_rectified_tanh(B, X, contact_subgraph)
...
# print("linear residual: " + str(linalg.norm(residual_linear)))
# print("ReLU residual: " + str(linalg.norm(residual_relu)))
# print("sigmoid residual: " + str(linalg.norm(residual_sigmoid)))
# print("tanh residual: " + str(linalg.norm(residual_tanh)))
# print("rectified tanh residual: " + str(linalg.norm(residual_rectified_tanh)))


min_val = predicted_O_rectified_tanh.min()                                          # Change
max_val = predicted_O_rectified_tanh.max()                                          # Change
absolute_max= max(abs(min_val), abs(max_val))
absolute_norm = TwoSlopeNorm(vmin = -absolute_max, vcenter=0, vmax = absolute_max)
# print("Min: " + str(predicted_O_linear.min()))
# print("Max: " + str(predicted_O_linear.max()))

plt.imshow(predicted_O_rectified_tanh, norm=absolute_norm, cmap="bwr")              #Change
plt.title("Rectified Tanh - Predicted O")                            #Change
plt.colorbar()


plt.savefig('../figures/predicted_O_rectified_tanh_test_2.png')                          #Change

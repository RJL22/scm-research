...
for i in range(sample_count):
	...
	O_samples.append(scm_optim.predict_O_ReLU(samp.generate_B(P), X, contact_subgraph))
...
predicted_rules = np.loadtxt("../output/predicted_rules/predicted_rules_ReLU_100sampleavg.csv", delimiter=",")

#Computing z-scores
z_scores = (predicted_rules - mean_O) / sd_O
print(z_scores)
#Saving data
np.savetxt("../output/statistics/ReLU_meanO_on_randomB_1000samples.csv", mean_O, delimiter=",")
np.savetxt("../output/statistics/ReLU_sdO_on_randomB_1000samples.csv", sd_O, delimiter=",")